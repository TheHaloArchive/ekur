...
use crate::{CommonCoating, CommonLayer, CommonRegion, CommonStyleList, CommonStyleListEntry};
...
pub fn process_material_coatings(
    ...
    HashMap<i32, CommonStyleList>,
    HashMap<String, CommonCoating>,
)> {
    let mut serde_styles = HashMap::new();
    let mut serde_stylelists = HashMap::new();
    for (id, style) in styles {
        ...
        for coating in &style.styles.elements {
            ...
            serde_styles.insert(format!("{id}_{}", coating.name.0), common_coating);
        ...
        serde_stylelists.insert(*id, style_list);
    }
    Ok((serde_stylelists, serde_styles))
}
