...
def _create_sockets(node_tree: NodeTree) -> None:
...
def _create_nodes(node_tree: NodeTree, uv: str) -> None:
    output = create_node(node_tree.nodes, 961, 158, NodeGroupOutput)
    mapping = create_node(node_tree.nodes, 796, 250, ShaderNodeMapping)
    combinexyz = create_node(node_tree.nodes, 208, 466, ShaderNodeCombineXYZ)
    combinexyz2 = create_node(node_tree.nodes, -392, -127, ShaderNodeCombineXYZ)
    input = create_node(node_tree.nodes, -972, 0, NodeGroupInput)
    mix = create_node(node_tree.nodes, -217, 212, ShaderNodeMix)
    mix2 = create_node(node_tree.nodes, 13, 205, ShaderNodeMix)
    add = create_node(node_tree.nodes, -15, -136, ShaderNodeMath)

    math_8 = create_node(node_tree.nodes, -393, -342, ShaderNodeMath)
    math_8.operation = "WRAP"
    assign_value(math_8, 1, 1.0)
    assign_value(math_8, 2, 0.0)

    subtract = create_node(node_tree.nodes, -214, -225, ShaderNodeMath)
    subtract.operation = "SUBTRACT"
    assign_value(subtract, 0, 1.0)

    multiply = create_node(node_tree.nodes, -588, -428, ShaderNodeMath)
    multiply.operation = "MULTIPLY"

    multiply2 = create_node(node_tree.nodes, -606, -156, ShaderNodeMath)
    multiply2.operation = "MULTIPLY"

    subtract2 = create_node(node_tree.nodes, -424, 59, ShaderNodeMath)
    subtract2.operation = "SUBTRACT"
    assign_value(subtract2, 0, 1.0)

    compare = create_node(node_tree.nodes, -402, 488, ShaderNodeMath)
    compare.operation = "COMPARE"
    assign_value(compare, 1, 0.5)
    assign_value(compare, 2, 0.0)

    compare2 = create_node(node_tree.nodes, -165, 522, ShaderNodeMath)
    compare2.operation = "COMPARE"
    assign_value(compare2, 1, 1.0)
    assign_value(compare2, 2, 0.0)

    uvmap = create_node(node_tree.nodes, 215, 134, ShaderNodeUVMap)
    uvmap.uv_map = uv
...
class BetterUVScaling:
    def __init__(self, uv: str = "UV0") -> None:
        group_name = "BetterUVScaling" if uv == "UV0" else f"BetterUVScaling{uv}"
        self.node_tree: NodeTree | None = bpy.data.node_groups.get(group_name)
        if self.node_tree:
            return
        self.node_tree = bpy.data.node_groups.new(type="ShaderNodeTree", name=group_name)
        _create_sockets(self.node_tree)
        _create_nodes(self.node_tree, uv)
